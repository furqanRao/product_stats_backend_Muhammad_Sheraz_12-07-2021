...
class SaleStatistics(APIView):
    ...
    def get(self, request):
        token = request.META.get('HTTP_AUTHORIZATION').split(' ')[1]
        current_user = Token.objects.filter(key=token).user
        products = ProductData.objects.all()
        current_user_products = products.objects.filter(user=current_user)

        current_user_sales_average = current_user_products.aggregate(Avg('sales_number')).values()[0]
        all_users_sales_average = products.aggregate(Avg('sales_number')).values()[0]
        highest_revenue_sale_for_current_user = current_user_products.objects.aggregate(Max('revenue'))[0]
